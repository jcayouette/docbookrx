...
The left navigation bar is the menu to the   from where you load the  pages.
...
Only  Administrators see the menu:Images[]
, menu:Users[]
, and menu:Admin[]
 entries. 
...
This section summarizes all of the categories and primary pages (those linked from the left navigation bar) within the  Web interface (). It does not list the many subpages, tabs and subtabs accessible from the individual pages.
...
Generate an activation key for a -entitled system.
...
Display and modify various aspects of autoinstallation profiles (Kickstart and ) used in provisioning systems. 
* {empty}
+
...
.menu:Software[]
View and manage the available  channels and the files they contain. 
...
.menu:Configuration[]
Keep track of and manage configuration channels, actions, individual configuration files, and systems with -managed configuration files. 
...
.menu:Systems[]
List the systems that have -managed configuration files. 
...
.menu:Admin[]
-- Use the Setup Wizard to configure . List, create, and manage one or more  organizations.
The  administrator can assign channel entitlements, create and assign administrators for each organization, and other tasks. 
...
.menu:Users[]
List all users known by , across all organizations.
Click individual user names to change administrative privileges of the user. 
+

[NOTE]
====
Users created for organization administration can only be configured by the organization administrator, _not_ the  administrator. 
====
* {empty}
+

.menu:Manager Configuration[]
Make General configuration changes to the  server, including Proxy settings, Certificate configuration, Bootstrap Script configuration, Organization changes, and Restart the  server. 
* {empty}
+
...
View the status of the various tasks of the  task engine. 
* {empty}
+

.menu:Show Tomcat Logs[]
Display the log entries of the Tomcat server, on which the  server is running. 
...
Throughout  you will see three patch (errata) alert icons. 
...
In the top bar,  offers a search functionality for Packages, Patches (Errata), Documentation, and Systems.
...
If you misspell a word during your search query, the  search engine performs approximate string (or fuzzy string) matching, returning results that may be similar in spelling to your misspelled queries. 

For example, if you want to search for a certain development system called `test-1.example.com` that is registered with , but you misspell your query ``tset``, the `test-1.example.com` system still appears in the search results. 

[NOTE]
====
If you add a distribution or register a system with a  server, it may take several minutes for it to be indexed and appear in search results. 
====
...
[NOTE]
====
Performing operations on large listssuch as removing RPM packages from the database with the  Web interfacemay take some time and the system may become unresponsive or signal "`Internal Server
