...
A summary of each patch is provided in list form displaying its type, advisory ID, synopsis (with the severity as a textual prefix in case of security updates, such as "`critical`"critical
, "`important`"important
, "`moderate`"moderate
, or "`low`"low
...
The menu:All[]
 patches page displays a list of all patches released by , irrelevant of whether they apply to your registered systems or not. 
...
[IMPORTANT]
====
If you use scheduled package installation, the packages or patches are installed via the  daemon (``rhnsd``). You must enable the  daemon on your systems.
For more information about the  daemon, see <<_bp.contact.methods.rhnsd>>
. 
====
...
This subtab displays the patch report issued by . It provides a synopsis of the patch first (with the severity as a textual prefix in case of security updates, such as "`critical`"critical
, "`important`"important
, "`moderate`"moderate
, or "`low`"low
...
More useful are the  Update Advisories at https://www.suse.com/support/update/. 
...
[WARNING]
====
If the organization is using both  and  Proxy server, then manage patches only on the  server since the proxy servers receive updates directly from it.
