...
== Systems
(((system list)))
((( Administrator,viewing a list of)))
...
* menu:Packages[] : Total number of package updates for the system, including packages related to patch alerts and newer versions of packages not related to patch alerts. For example, if a client system that has an earlier version of a package installed gets subscribed to the appropriate base channel (such as  12 SP2), that channel may have an updated version of the package. If so, the package appears in the list of available package updates. 
+

.Package Conflict
[IMPORTANT]
====
If  identifies package updates for the system, but the package updater (such as Red Hat Update Agent or ) responds with a message like Your system is fully updated, a conflict likely exists in the system's package profile or in the [path]_up2date_
 configuration file.
...
You have permission if you are the only user in your organization, if you are a  Administrator, or if the system belongs to a group for which you have admin rights. 
...
To reach this page, select menu:Systems[Systems > Physical Systems]
 from the left bar.
This page lists each physical system of which  is aware. 
...
To reach this page, select menu:Systems[Systems > Virtual Systems]
 from the left bar.
This page lists each virtual host of which  is aware and the guest systems on those hosts. 
...
Only guests registered with  are displayed with blue text.
...
Non-compliant systems have packages installed which are not available from . menu:Packages[]
...
The menu:Inactive Systems[]
 page displays systems that have not checked in with  for 24 hours or more. 


image::systems_inactive.png[]


Checking in means that Zypper on  or Red Hat Update Agent on  client systems connects to  to see if there are any updates available or if any actions have been scheduled.
If you see a message telling you that check-ins are not taking place, the client system is not successfully connecting to . The reason may be one of the following: 

* The system is not entitled to any  service. System profiles that remain unentitled for 180 days (6 months) are removed. 
* The system is entitled, but the  daemon ([path]_rhnsd_ ) has been disabled on the system. Refer to <<_bp.contact.methods.rhnsd>> for instructions on restarting and troubleshooting. 
...
* The system is connected to a  Proxy Server or  that has not been properly configured. 
* The system itself has not been properly configured, perhaps pointing at the wrong  Server. 
* The system is not in the network. 
* Some other barrier exists between the system and the  Server. 
...
The menu:Proxy[]
 page displays the  Server systems registered with your  server. 
...
Active entitlements are in gray, while inactive entitlements are highlighted in yellow and their check boxes checked by default for you to delete them as needed by clicking the menu:Delete Selected[]
 button.
Entitlements are inactive if the system has not checked in with in a time specified via the drop-down box menu:A system
...
The report can be used to prioritize maintenance actions on the systems registered to . 
...
System Types define the set of functionalities available for each system in such as the ability of installing software or creating guest virtual machines. 
...
When systems are registered to , they are displayed on the menu:Systems[Overview]
 page.
...
It does, however, remove your ability to manage it via . 

If you mistakenly deleted a system profile from , you may re-register the system using the bootstrap script or `rhnreg_ks` manually. 
====
...
System ID::
A unique identifier generated each time a system registers with . 
+


[NOTE]
====
The system ID can be used to eliminate duplicate profiles from . Compare the system ID listed on this page with the information stored on the client system in the [path]_/etc/sysconfig/rhn/systemid_
 file.
...
For example, if a user logs in to the client directly and runs  Online Update (on ) or `pup` (on ), the update tool will install available patches even if the system is locked in the Web interface. 
...
An example is the menu: Tools[]
channel. 

.menu:System Events[]
Checked In::
The date and time at which the system last checked in with . 

Registered::
The date and time at which the system registered with  and created this profile. 
...
When the client checks in after the scheduled start time,  will instruct the system to restart itself. 
====
...
+
The so-called OSA status is also displayed for client systems registered with  that have the OSA dispatcher (osad) configured. 
+
Push enables  customers to immediately initiate tasks rather than wait for those systems to check in with . Scheduling actions through push is identical to the process of scheduling any other action, except that the task can immediately be carried out instead of waiting the set interval for the system to check in. 
+
In addition to the configuration of , to receive pushed actions each client system must have the [path]_osad_
...
==== System Details DetailsProperties
...
==== System Details DetailsRemote Command
...
. On  clients, subscribe the system to the  Tools child channel. Then use Zypper to install the ``rhncfg``, ``rhncfg-client``, and `rhncfg-actions` packages, if not already installed: 
+
...
----
+
On  clients, subscribe the system to the Tools child channel.
...
----
. Log in to the system as root and add the following file to the local configuration directory: [path]_allowed-actions/scripts/run_ . 
+
...
----
** Create an empty [path]_run_ file in that directory to act as a flag to , signaling permission to allow remote commands: 
+
...
==== System Details DetailsReactivation[Management]
(((reactivating,systems)))


Reactivation keys include this system's ID, history, groups, and channels.
This key can then be used only once with the `rhnreg_ks` command line utility to re-register this system and regain all  settings.
...
[WARNING]
====
When autoinstalling a system with its existing  profile, the profile uses the system-specific activation key created here to re-register the system and return its other  settings.
...
==== System Details DetailsHardware
...
If the hardware profile looks incomplete or outdated, click the menu:Schedule Hardware
     Refresh[]
 button.
The next time the  Daemon ([path]_rhnsd_
) connects to , it will update your system profile with the latest hardware information. 

[[_s5_sm_system_details_migrate]]
==== System Details DetailsMigrate
...
==== System Details DetailsCustom Info
...
==== System Details DetailsProxy[Proxy]


This tab is only available for  systems.
The tab lists all clients registered with the selected  server. 
...
==== System Details SoftwarePackages
...
[WARNING]
====
When new packages or updates are installed on the client via , any licenses (EULAs) requiring agreement before installation are automatically accepted. 
====
...
The next time the  daemon ([path]_rhnsd_
) connects to , it updates your system profile with the latest list of installed packages. 
...
==== System Details SoftwareSoftware Channels
...
==== System Details SoftwareSP Migration
...
[WARNING]
====
During migration  automatically accepts any required licenses (EULAs) before installation. 
====


Beginning with SLE 12  supports service pack skipping, it is now possible to migrate from for example, SLE 12 to SLE 12 SP2.
...
==== System DetailsConfigurationOverview
...
==== System Details ConfigurationView/Modify Files
...
==== System Details ConfigurationAdd Files
...
enter the full path of any files you want import into  or select deployable configuration files from the menu:Import Existing
Files[]
list.
...
==== System Details ConfigurationDeploy Files
...
==== System Details ConfigurationCompare Files

##emap 2014-04-22: Still accurate?#
 This subtab compares a configuration file stored on the  with the file stored on the client.
...
==== System DetailsConfigurationManage Configuration Channels
...
The menu:Provisioning[]
 tab and its subtabs allow you to schedule and monitor  or Kickstart installations and to restore a system to its previous state. 

is a  and Kickstart is a Red Hat utilityboth allow you to automate the reinstallation of a system.
...
==== System DetailsProvisioningAutoinstallation
...
In this case  and its network must be properly configured to allow boot using PXE. 
...
For example, to  a system that joins the network of a specific organization (for example the Engineering department) you can create a profile variable to set the IP address and the gateway server address to a variable that any system using that profile will use.
...
For example, the [option]``network`` portion of a  file could look like the following: 
...
[NOTE]
====
There is a hierarchy when creating and using variables in  files.
...
==== System DetailsProvisioningPower Management
...
You need a fully patched  installation.
To use any power management functionality, IPMI configuration details must be added to . First select the target system on the systems list, then select menu:Provisioning[Power
     Management]
.
...
Those are not supported by  but can be used by adding the fence agent names as a comma-separated list to the `java.power_management.types` configuration parameter. 

[[_s4_sm_system_details_shots]]
==== System DetailsProvisioningSnapshots


Snapshots enable you to roll back the system's package profile, configuration files, and  settings. 
...
[NOTE]
====
There is no maximum number of snapshots that  will keep, thus related database tables will grow with system count, package count, channel count, and the number of configuration changes over time.
...
==== System Details ProvisioningSnapshot Tags
...
==== System DetailsVirtualizationDetails
...
[NOTE]
====
If a guest system has not registered with , this information appears as plain text in the table. 
====
...
((( Administrator,assigning and removing)))
...
==== System DetailsGroupsList/Leave
...
Only System Group Administrators and  Administrators can remove systems from groups.
...
Only System Group Administrators and  Administrators can add a system to groups.
Non-admins see a menu:Review this system's group
     membership[]
 page.
...
==== System Details StatesHighstate
...
((( Administrator,System Group List)))
(((Web Interface,System Groups)))


The menu:System Groups[]
 page allows  users to view the menu:System Groups[]
 list. 
...
Only System Group Administrators and  Administrators may perform the following additional tasks: 
...
((( Administrator,adding and removing)))
...
Clicking it does not delete systems from  entirely.
...
The menu:States[]
 tab displays states which have been created and added using the menu:Salt[]menu:State
     Catalog[]
.
...
==== System Set Manager PackagesInstall
...
==== System Set Manager PackagesRemove
...
==== System Set Manager PackagesUpgrade
...
[[_s3_sm_ssm_verify]]
==== System Set Manager PackagesVerify
...
These functions are limited to  Administrators and System Group Administrators.
...
==== System Set ManagerChannelsBase Channels
...
==== System Set Manager ChannelsChild Channels
...
==== System Set ManagerConfigurationDeploy Files


Use this subtab to distribute configuration files from your central repository on  to each of the selected systems. 
...
==== System Set Manager ConfigurationCompare Files


Use this subtab to validate configuration files on the selected systems against copies in your central repository on . 
...
To compare the configuration files deployed on the systems with those in , select the check box for each file to be validated.
Then click menu:Analyze Differences[Schedule File
     Comparison]
.
The comparisons for each system will not complete until each system checks in to . When each comparison is complete, any differences between the files will be accessible from each system's events page. 
...
==== System Set Manager ConfigurationSubscribe to Channels
...
This tab is available only to  Administrators and Configuration Administrators. 
...
==== System Set Manager ConfigurationUnsubscribe from Channels
...
==== System Set Manager ConfigurationEnable Configuration
...
Then the systems will get subscribed to the required  tools channel and required [package]#rhncfg*#
...
==== System Set ManagerProvisioningAutoinstallation
...
If such a profile cannot be found,  will look for an organization default profile and apply that instead. If no matching IP ranges nor organization default profiles can be found, no autoinstallation will be performed on the system.
...
For more information on Cobbler and templates, refer to <<_advanced.topics.cobbler>>
. 
...
If any of the systems connect to  via a proxy server, choose either the menu:Preserve Existing Configuration[]
...
==== System Set Manager ProvisioningTag Systems
...
==== System Set Manager ProvisioningRollback
...
==== System Set Manager ProvisioningRemote Command
...
==== System Set Manager ProvisioningPower Management Configuration
...
==== System Set Manager ProvisioningPower Management Operation
...
System sets can be scheduled for XCCDF scans; XCCDF stands for "`The
    Extensible Configuration Checklist Description Format`"The
...
==== System Set ManagerMiscHardware
...
==== System Set Manager MiscSoftware
...
==== System Set Manager MiscMigrate
...
==== System Set Manager MiscLock/Unlock
...
The menu:Bootstrapp Minions[]
 page allows you to bootstrap Salt minions from the . 
...
As of  3.1 you can visualize your virtualized, proxy, and systems group topologies.
...
The following is an example graphical representation tree of the virtual network hierarchy of virtual systems registered with . 
...
The following is an example graphical representation tree of the proxy network hierarchy of proxy systems and their clients registered with . 
...
The following is a graphical representation tree of the all systems registered with . 
...
Users with the Activation Key Administrator role (including  Administrators) can generate activation keys in the  Web interface.
With such an activation key, register a  or Red Hat Enterprise Linux system, entitle the system to a  service level and subscribe the system to specific channels and system groups through the `rhnreg_ks` command line utility. 
...
....... menu:Key[] -- Either choose automatic generation by leaving this field blank or enter the key you want to generate in the menu:Key[] field. This string of characters can then be used with `rhnreg_ks` to register client systems with . Refer to <<_s3_sm_system_keys_multiple>> for details. 
+
...
....... menu:Base Channels[] -- The primary channel for the key. This can be either the ` Default` channel, a provided channel, or a custom base channel. 
+ 
Selecting ` Default` allows client systems to register with the -provided default channel that corresponds with their installed version of . You can also associate the key with a custom base channel.
If a system using this key is not compatible with the selected channel, it will fall back to the  default channel. 
....... menu:Add-on System Types[] -- The supplemental system types for the key, e.g. Virtualization Host. All systems will receive these system types with the key. 
....... menu:Contact Method[] - Select how clients communicate with . menu:Default[] (Pull) waits for the client to check in. With menu:Push via SSH[] and menu:Push via SSH tunnel[] the server contacts the client via SSH (with or without tunnel) and pushes updates and actions, etc. 
+ 
...
Separate keys with a comma at the command line with `rhnreg_ks` or in a profile in the menu:Activation Keys[]
...
Third party hypervisors and hypervisor managers such as VMWare vCenter are called "`Virtual Host Managers`"Virtual Host Managers
 (VHM) within . These managers can manage one or multiple virtual hosts, which in turn may contain virtual guests.
ships with a tool called `virtual-host-gatherer` that can connect to VHMs using their API, and request information about virtual hosts.
...
For more information, see the man page on your  server for ``virtual-host-gatherer``: 
